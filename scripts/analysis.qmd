...
incl_id_hc <- readRDS("behaviour/processed-data/incl_id_hc.rds")
incl_id_mpfc <- readRDS("behaviour/processed-data/incl_id_mpfc.rds")
incl_id_lc <- readRDS("behaviour/processed-data/incl_id_lc.rds")
...
# ~~~~ DEMOGRAPHICS ~~~~ ---- 
demogr <- read_csv("data/demographics/All_participants_data_index.csv") %>%
	rename(id = ID_Code, group = `Group SD mPFC`, age = Age, gender = Gender) %>% 
	dplyr::select(id, group, age, gender) %>%
	mutate(id = if_else(id == 2012, 299, id)) %>% 
	filter(id < 600) %>%
...
# ~~~~ QUESTIONNAIRES ~~~~ ---- 
qnr <- read_csv("data/demographics/All_participants_combined_data_24-Apr-2020_totals.csv") %>% 
	rename(id = ID_Code, 
				 iq = WTAW_Standard, 
				 trail_a = `Trail_Making_A_(seconds)'`, 
				 trail_b = `Trail_Making_B_(seconds)'`, 
				 ...
				 bdi_total = BDI_total, 
				 edu = Education_years) %>% 
	dplyr::select(id, iq, trail_a, trail_b, ami_total, ami_ba, ami_sm, ami_es, bdi_total, edu, SD_learn_p1, SD_learn_p2, SD_similar_p1, SD_similar_p2) %>% 
	mutate(id = if_else(id == 2012, 299, id)) %>% 
	filter(id < 600) %>% 
	filter(id %in% incl_id) %>% 
	mutate(iq = as.numeric(iq), 
				 ...
				 SD_learn_p1 = replace(SD_learn_p1, is.nan(SD_learn_p1), NA), 
				 SD_learn_p2 = replace(SD_learn_p2, is.nan(SD_learn_p2), NA), 
				 SD_similar_p1 = replace(SD_similar_p1, is.nan(SD_similar_p1), NA),
				 SD_similar_p2 = replace(SD_similar_p2, is.nan(SD_similar_p2), NA),
				 )

# ~~~~ KL DIVERGENCE ~~~~ ----
load("behaviour/processed-results/ku0_kld_hc.RData")
kld_hc <- result_tb
load("behaviour/processed-results/ku0_kld_mpfc.RData")
kld_mpfc <- result_tb
load("behaviour/processed-results/ku0_kld_lc.RData")
kld_lc <- result_tb
rm(result_tb)
