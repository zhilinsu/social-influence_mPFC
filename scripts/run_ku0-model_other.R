...
# ============================================================================.
	for (o_block in c("i", "p")) { # other blocks 
		# 1 - Construct data
		...
		for (i in 1:n_pt) {
			self_decision[i, ] <- pt_new[, paste("so_dec_", o_block, sep = ""), i] 
			reward_sooner[i, ] <- pt_new[, paste("o_rs_", o_block, sep = ""), i]
			reward_later[i, ] <- pt_new[, paste("o_rl_", o_block, sep = ""), i]
			delay_later[i, ] <- pt_new[, paste("o_dl_", o_block, sep = ""), i] 
		}
		...
		cat("Estimating for the Block", o_block, "... \n")
		start_time <- Sys.time()
		print(start_time)
		...
		cat("Finishing the estimation for the Block", o_block, "... \n")
		end_time <- Sys.time()
		print(end_time)  
		...
		if (pop == 0) {
			file_name <- paste("stanfit_ku0_all_other_", o_block, ".rds", sep = "")
		} else if (pop == 1) {
			file_name <- paste("stanfit_ku0_hc_other_", o_block, ".rds", sep = "")
		} else if (pop == 3) {
			file_name <- paste("stanfit_ku0_mpfc_other_", o_block, ".rds", sep = "")
		} else if (pop == 4) {
